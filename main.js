...
function updateGamepadState() {
  ...
  gamepad.lastButtons = [...gamepad.buttons]; 
...
function isGamepadButtonPressed(buttonIndex) {
  ...
  return gamepad.buttons[buttonIndex] && !gamepad.lastButtons[buttonIndex];
}
...
function update(dt) {
  ...
  if (gamepad.connected) {
    ...
    if (Math.abs(axisX) > GAMEPAD_CONFIG.DEADZONE) {
      dx = axisX;
    }
    if (Math.abs(axisY) > GAMEPAD_CONFIG.DEADZONE) {
      dy = axisY;
    }
  }
  ...
  resolveColl(player);
    ...
    if (Math.hypot(aimX, aimY) > GAMEPAD_CONFIG.DEADZONE) {
      gamepad.isAiming = true;
      gamepad.aimAngle = Math.atan2(aimY, aimX);

      mouse.x = (player.x - camera.x) + Math.cos(gamepad.aimAngle) * GAMEPAD_CONFIG.AIM_SENSITIVITY;
      mouse.y = (player.y - camera.y) + Math.sin(gamepad.aimAngle) * GAMEPAD_CONFIG.AIM_SENSITIVITY;
    } else {
      
      gamepad.isAiming = false;
    }
  }
  
  const isShooting = mouse.down || (gamepad.connected && gamepad.buttons[7]); 
  if (isShooting) shoot();
  
    if(gamepad.connected) {
      
      if (isGamepadButtonPressed(0) && player.dash.cooldown <= 0 && !resetInProgress) {
          
          const angle = gamepad.isAiming ? gamepad.aimAngle : angleBetween(player, {x: mouse.x + camera.x, y: mouse.y + camera.y});
      ...
      if (isGamepadButtonPressed(5)) {
        ...
        player.weapon = weaponKeys[nextIndex];
      }
      
      
      if (isGamepadButtonPressed(4)) {
        const prevIndex = (currentIndex - 1 + weaponKeys.length) % weaponKeys.length; 
        player.weapon = weaponKeys[prevIndex];
      }
  }
...
function draw() {
  ...
  const angle = gamepad.isAiming ? gamepad.aimAngle : Math.atan2(worldMouseY - player.y, worldMouseX - player.x);
...
function setupEventListeners() {
  ...
  window.addEventListener("gamepadconnected", (e) => {
    console.log(
      "Gamepad conectado no índice %d: %s. %d botões, %d eixos.",
      e.gamepad.index,
      e.gamepad.id,
  ...
  window.addEventListener("gamepaddisconnected", (e) => {
    console.log("Gamepad desconectado do índice %d: %s", e.gamepad.index, e.gamepad.id);
